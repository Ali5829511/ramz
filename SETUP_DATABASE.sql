...
-- ملاحظة: لا يوجد UNIQUE على اسم العقار لأن نفس الصك قد يكون له أكثر من سجل (وحدات مختلفة).
-- فقط رقم العقد ورقم الفاتورة مقيدان بـ UNIQUE (محدد في تعريف الجدول).

-- منع تكرار الوحدة (نفس رقم الوحدة في نفس العقار)
DO $$
BEGIN
  IF NOT EXISTS (
...
CREATE INDEX IF NOT EXISTS idx_properties_deed ON properties ("رقم_الصك");
